...
def build_whatsapp_backup(*args, **kwargs):
    from backend.domain.runtime.reports import build_whatsapp_backup as _impl

    return _impl(*args, **kwargs)


def build_whatsapp_rds(*args, **kwargs):
    from backend.domain.runtime.reports import build_whatsapp_rds as _impl

    return _impl(*args, **kwargs)


def build_whatsapp_alarm(*args, **kwargs):
    from backend.domain.runtime.reports import build_whatsapp_alarm as _impl
...
def huawei_time_range_short(row: dict[str, Any]) -> str:
    ...
    if isinstance(start, int) and isinstance(peak, int) and start < peak:
        start_txt = (
            datetime.fromtimestamp(start / 1000).astimezone().strftime("%H:%M:%S")
        )
        peak_txt = (
            datetime.fromtimestamp(peak / 1000).astimezone().strftime("%H:%M:%S %Z")
        )
...
def classify_huawei_memory_behavior(row: dict[str, Any], rise_threshold: float) -> str:
    ...
    if avg_v is not None and avg_v >= rise_threshold and (peak_v - avg_v) <= 5.0:
        return "HIGH_STABLE"
    if (
        latest_v is not None
        and latest_v >= rise_threshold
        and (peak_v - latest_v) <= 3.0
    ):
        return "HIGH_STABLE"
...
def build_huawei_utilization_customer_report(
    ...
    for row in util.get("top_mem_hot") or []:
        ...
        behavior = row.get("behavior") or classify_huawei_memory_behavior(
            row, rise_threshold
        )
        if behavior == "HIGH_STABLE":
            stable.append(item)
...
def build_huawei_legacy_consolidated_report(
    ...
    for profile in ordered_profiles:
        ...
        cpu_host = cpu_row.get("name", "-")
        cpu_peak = cpu_row.get("peak")
        cpu_has_data = isinstance(cpu_peak, (int, float)) and isinstance(
            cpu_avg, (int, float)
        )
        ...
        mem_peak = mem_row.get("peak")
        mem_host = mem_row.get("name", "-")
        mem_has_data = isinstance(mem_peak, (int, float)) and isinstance(
            mem_avg, (int, float)
        )
        ...
        if not mem_has_data:
            ...
            if not isinstance(mem_peak, (int, float)):
                lines.append(
                    "   Data utilisasi memory tidak tersedia pada periode ini."
                )
                continue
