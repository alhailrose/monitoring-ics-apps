...
d = r.json()
check(
    "aryanoble 17 profiles", d.get("total_profiles") == 17, str(d.get("total_profiles"))
)
...
connect_prod_id = next(
    (a["id"] for a in accts if a["profile_name"] == "connect-prod"), None
)
...
check(
    "report has account",
    "Connect" in d.get("report", "") or "connect-prod" in d.get("report", ""),
)
