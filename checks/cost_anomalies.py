...
class CostAnomalyChecker(BaseChecker):
    ...
    def format_report(self, results):
        ...
        if results['total_anomalies'] == 0:
            lines.append(f"Cost monitoring completed. {results['total_monitors']} monitor(s) active.")
            lines.append("No cost anomalies detected in the last 2 days.")
        else:
            lines.append(f"Cost monitoring completed. {results['total_monitors']} monitor(s) active.")
            lines.append(f"{results['total_anomalies']} cost anomalies detected requiring review.")
        ...
        for monitor in results['monitors']:
            ...
            lines.append(f"  Services Tracked: {monitor.get('DimensionalValueCount', 0)}")
        ...
        if results['anomalies']:
            ...
            if results['total_anomalies'] > 0:
                total_impact = sum(float(a.get('Impact', {}).get('TotalImpact', 0)) for a in results['anomalies'])
                lines.append(f"Status: {results['total_anomalies']} anomalies detected (Total Impact: ${total_impact:.2f})")
            else:
            ...
            for idx, anomaly in enumerate(results['anomalies'], 1):
                ...
                lines.append(f"  Date Range: {start_date} to {end_date} ({duration_days} days)")
                lines.append(f"  Anomaly Score: {anomaly.get('AnomalyScore', {}).get('CurrentScore', 'N/A')}/100")
                lines.append("")
                ...
                lines.append(f"    Total Actual Spend: ${float(impact.get('TotalActualSpend', 0)):.2f}")
                lines.append(f"    Total Expected Spend: ${float(impact.get('TotalExpectedSpend', 0)):.2f}")
                lines.append(f"    Impact Percentage: {impact.get('TotalImpactPercentage', 0)}%")
                ...
                if daily_breakdown:
                    ...
                    for day_data in daily_breakdown:
                        ...
                        day_cost = float(day_data.get('Total', {}).get('UnblendedCost', {}).get('Amount', 0))
                ...
                if root_causes:
                    ...
                    for rc_idx, rc in enumerate(root_causes, 1):
                        ...
                        lines.append(f"    {rc_idx}. Service: {rc.get('Service', 'N/A')} (Impact: ${contribution:.2f})")
                        lines.append(f"       Region: {rc.get('Region', 'N/A')}")
                        if rc.get('UsageType'):
                            lines.append(f"       Usage Type: {rc.get('UsageType')}")
                        if rc.get('LinkedAccount'):
                            account_name = rc.get('LinkedAccountName', rc.get('LinkedAccount'))
        ...
        rec_count = 1
        if results['total_anomalies'] > 0:
            high_impact = [a for a in results['anomalies'] if float(a.get('Impact', {}).get('TotalImpact', 0)) > 50]
            if high_impact:
                lines.append(f"{rec_count}. IMMEDIATE REVIEW: Investigate high-impact anomalies")
                lines.append(f"   {len(high_impact)} anomaly(ies) with impact > $50")
                rec_count += 1
            
            lines.append(f"{rec_count}. COST ANALYSIS: Review root causes and optimize spending")
            lines.append(f"   Check Cost Explorer for detailed breakdown")
            rec_count += 1
        else:
            lines.append(f"{rec_count}. ROUTINE MONITORING: Continue cost anomaly monitoring")
