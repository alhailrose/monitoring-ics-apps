...
def _check_profile_health(profile_name: str) -> ProfileStatus:
    ...
    try:
        ...
        msg = str(exc).lower()
        if any(hint in msg for hint in ("expired", "token", "sso", "unable to load sso")):
...
class SessionHealthService:
    ...
    def check_all(self, customer_id: str | None = None) -> SessionHealthReport:
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(_check_profile_health, p): p
    ...
    def check_and_notify(
        ...
        for sess_name, sess_info in report.sso_sessions.items():
            if sess_info["status"] != "ok":
                lines.append(f"SSO Session: {sess_name} [{sess_info['status'].upper()}]")
                lines.append(f"  Fix: {sess_info['login_command']}")
                if sess_info["profiles_expired"]:
                    lines.append(f"  Expired profiles: {', '.join(sess_info['profiles_expired'])}")
                if sess_info["profiles_error"]:
                    lines.append(f"  Error profiles: {', '.join(sess_info['profiles_error'])}")
                lines.append("")
        ...
        if slack_webhook_url:
            sent, reason = send_to_webhook(slack_webhook_url, message, channel=slack_channel)
            if sent:
